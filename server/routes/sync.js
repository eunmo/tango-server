(function() {
  ...
  module.exports = function(router, mysql) {
    router.put('/sync', async function(req, res) {
      ...
      const query =
        'SELECT * FROM words WHERE `streak` < 11 ORDER BY `level`, `index`';
      var data = await mysql.promisifyQuery(query);

      var map = {};
      data.forEach(word => {
        map[word.level + word.index] = word;
      });
      ...
      input.forEach(word => {
        ...
        if (storedWord === undefined) return;
        ...
        if (storedDate !== inputDate) {
          ...
          if (word.streak === 11) learnedCount++;
        }
      });
      ...
      if (updates.length > 0) {
        ...
        if (learnedCount > 0) data = await mysql.promisifyQuery(query);
      }
